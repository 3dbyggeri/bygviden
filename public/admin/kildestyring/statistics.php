...
$docStats = $statistics->getDocStats($from,$to);
$docDayStats = $statistics->getDocDayStats($to);
$from_date = new date_widget('from',$from['d'],$from['m'],$from['Y']);
$to_date = new date_widget('to',$to['d'],$to['m'],$to['Y']);
$docsOverview = $statistics->getDocsOverview($from,$to);
$last24 = $statistics->getLast24HoursDokuments($to);
$topReaded = $statistics->getTopReadedDokuments($from,$to);
$topReadedVidensLev = $statistics->getTopReadedVidensLev($from,$to);
$graphHeight = 400;
?>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  ...
  <tr>
    <td class="header">
      Kilde statistik <span class="alert_message"><?=$message?></span>
    </td>
  </tr>   
  <tr> 
    <td align="center">&nbsp;</td>
  ...
  <tr> 
      <td>
        <table width="100%" cellpadding="0" cellspacing="0" border="0" class="color1">
            ...
            <tr>
                <td class="tdpadtext">Antal læste dokumenter i <?=($periode)?'perioden':'de sidste 30 dage'?>:</td>
                <td class="alert_message" style="font-size:12px">[<?=$docStats?>]</td>
            </tr>	
            <tr>
                <td class="tdpadtext">Antal læste dokumenter indenfor de 
                                      sidste 24 timer <?=($periode)?'af perioden':''?>:</td>
            ...
            <tr>
                <td class="tdpadtext">Læste dokumenter <?=$period_label?></td>
            ...
            <tr> <td colspan="2" class="tdpadtext">&nbsp;</td> </tr>	
            <tr>
                <td class="tdpadtext">Læste dokumenter <?=$day_label?></td>
                <td class="alert_message">&nbsp;</td>
            </tr>	
            <tr>
              <td colspan="2" class="tdpadtext">
            ...
            <tr>
                <td class="tdpadtext" 
                  colspan="2"><a name="topvisit">Mest læste videns leverandør 
                  <?=($periode)? 'i perioden':'de sidste 30 dage'?></td>
            </tr>	
            <tr>
                <td colspan="2">
                  <table width="100%" cellpadding="3" cellspacing="0" border="0">
                    <?if( !count( $topReadedVidensLev ) ):?>
                       <tr class="color2" style="height:30px">
                          <td class="tabelText" colspan="2" align="center">No data available</td>
                       <tr> 
                    <?endif?>
                    <?for( $i = 0; $i < count( $topReadedVidensLev ); $i++ ):?>
	                   <tr class="<?=($i%2==0)?"color2":"color3"?>">
                        <td style="font-size:10px;padding-left:10px"> <?=$topReadedVidensLev[$i]['viden_lev_name']?> </td>
                        <td class="plainText"><?=$topReadedVidensLev[$i]["total"]?></td>
                     </tr>
                    <?endfor?>
                  </table>
                </td>
