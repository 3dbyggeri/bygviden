...
class bruger 
{
    ...
    function hasPaidBrugerRabat($kilde,$url)
    {
      ...
      $kid = $kilde->findRelevantBrugerRabatKildeId();
      ...
      $sql ="SELECT
                ...
                WHERE
                  us.publication_id = pub.id
                AND
                  pub.parent = cat.id
                AND
                  cat.parent = lev.id
                AND
                  ( $kid = pub.id OR $kid = cat.id OR $kid = lev.id )
                AND
                  us.bruger_id=". $uid."
                AND
                  TO_DAYS(NOW()) - TO_DAYS(us.readed) < ". ( 30 * 12 ) ." ";
    ...
    function hasPaid( $kilde, $url )
    {
      ...
      if($kilde->hasBrugerRabat()) return $this->hasPaidBrugerRabat($kilde,$url);
