			...
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
                  ...
                  ?>
									...
									<?elseif( $move ):?>
										<a href="<?=$PHP_SELF?>?move=<?=$move?>&where=<?=$nodes[$i]["id"]?>" class="nodeName" onfocus="if(this.blur)this.blur();"><img src="graphics/doc.gif" alt="Edit" border="0"/></a>
									<?else:?>
										<a href="../building/index.php?id=<?=$nodes[$i]["id"]?>" title="Edit" class="nodeName" onclick="parent.resize('normal');" target="contentfrm" onfocus="if(this.blur)this.blur();"><img src="graphics/doc<?=$iconPostFix?>.gif" alt="Edit" border="0"/></a>
									<?endif?>
								</td>
								<?//==================SPACER CELL============================?>
