...
<html>
    ...
    <body bgcolor="#FFFFFF" 
          onload="restoreScrollPosition('<?=urlencode($_SERVER['PHP_SELF'] )?>')"  
          onunload="saveScrollPosition( '<?=urlencode($_SERVER['PHP_SELF'])?>')">
			...
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
                  ...
                  ?>
									...
									<?elseif( $move ):?>
										<a href="<?=$PHP_SELF?>?move=<?=$move?>&where=<?=$nodes[$i]["id"]?>" class="nodeName" 
                      onfocus="if(this.blur)this.blur();"><img src="graphics/doc.gif"  border="0"/></a>
									<?else:?>
					...
					<td onmouseover="shownode( <?=$nodes[$i]["id"]?> )"  onmouseout="hidenode( <?=$nodes[$i]["id"]?> )">
						<table border="0" cellpadding="0" cellspacing="0" class="hide" id="<?=$nodes[$i]["id"]?>">
							<tr>
								...
								<td>
										<a title="Delete" href="javascript:removing( <?=$nodes[$i]["id"]?> )" 
                      onmouseover="lightup('delete', <?=$nodes[$i]["id"]?>)" 
                      onmouseout="grayout('delete', <?=$nodes[$i]["id"]?>)" 
                      onfocus="if(this.blur)this.blur();"><img 
                      src="graphics/delete_off.gif" 
                      name="delete<?=$nodes[$i]["id"]?>" border="0" alt="Delete"></a>
								</td>
								<td>
