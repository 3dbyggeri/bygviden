...
class Produkter extends View 
{
  ...
  function renderAlphabet()
  {
    ...
    foreach($this->alfabet as $key=>$value) $a.='<td><a href="#'.$key.'">'.$key.'</a></td>';
    return $a .'</tr></table>'; 
  }
  function loadPage()
  {
  ...
  function renderProduct($product)
  {
    ...
    {
      $str.='<img src="/logo/'.$product['logo_url'].'" align="top" 
  ...
  function getProducent($id)
  {
    ...
    {
      $s.='<img src="/logo/'.$this->producer['logo_url'].'" align="top" 
  ...
  function producenter()
  {
    ...
    foreach($this->alfabet as $key=>$value)
    {
      ...
      for($i=0;$i<count($value);$i++)
      {
        ...
        $s.='<li><a href="http://www.bygviden.dk/'.$this->page.$this->action.'&section='.
  ...
  function vareGrupper()
  {
    ...
    foreach($this->alfabet as $key=>$value)
    {
      ...
      for($i=0;$i<count($value);$i++)
      {
        ...
        if($hasProducts)
        {
            $s.=' <span>['.  $productCountForVareGroup[ $value[$i]['id'] ].  ']</span>';
        }
        $s.='</li>';
  ...
  function producter()
  {
    ...
    {
      if(!count($value)) continue; 
      $s.= '<a name="'.$key.'" /><div class="category"><a class="tiltop" href="javascript:toppen()">til top &uarr; </a> '.$key.'</div>'; 
  ...
  function LeftMenu()
  {
    ...
    foreach($this->menu as $key=>$value)
    {
      ...
      $str.= '<li><a href="http://www.bygviden.dk/'.$this->page.$this->action.'&section='.$key.'" '.$sel.'>'. $value .'</a></li>';
    }
