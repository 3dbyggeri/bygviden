  ...
  function loadPublications($lev)
  {
    ...
    $str = '
    		...
    		function toggleCat(id)
		{
			...
			var els = getElementsByClass(\'cat_\'+id,document.getElementById(\'posts\'),\'tr\');
			for(var i=0;i < els.length;i++)
			{
				els[i].style.display = (el.src.indexOf(\'right\') > 0 )? \'\':\'none\';
			}

			el.src = (el.src.indexOf(\'right\') > 0 )? \'graphics/arrow_down.gif\':\'graphics/arrow_right.gif\';
		}
	    </script>';
    ...
    foreach($cats as $key=>$value)
    {
      ...
      {
      	$img = '<a href="javascript:toggleCat(\''. $key.'\')"><img src="graphics/arrow_right.gif" align="absbottom" border="0" id="img_'. $key.'"/></a>';
	$count_label = (count($sorted) ==1)? 'publikation':'publikationer';
	$pub_count = '<span style="font-weight:100;font-size:10px;color:#333;float:right">('. count($sorted) .' '. $count_label.')</span>';
	$str.= '<script> $(document).ready(function(){ $(".cat_'.$key.'").hide(); }); </script>';
      }
      $str.= '<tr >
		    <td colspan="2" align="left" class="kategori" style="background-color:#fff">
